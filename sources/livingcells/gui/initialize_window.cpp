...
initialize_window::initialize_window(QWidget *parent, Api *a, Field *f, int i, int j) : QDialog(parent)
{
    ...
    status_cell->setText(QString("Выберете состояние клетки %1 строки, %2 столбца").arg(y+1).arg(x+1));
    status_cell->show();
    live_button = new QPushButton("Живая", this);
    live_button->resize(BUTTON_SIZE);
    live_button->move(WINDOW_SIZE.width() - 325, WINDOW_SIZE.height() - 50);
    ...
    dead_button->move(WINDOW_SIZE.width() - 325, WINDOW_SIZE.height() - 100);
